...
static sharedbuffer_t *initbuffer() {
    ...
    /* the default behavior of pthread mutex is that mutexes are shared
     * among threads within the same process, which is not suitable
     * for synchronizes processes. Therefore, in the following, we 
     * need to initialize the attributes of a mutex to be shared among
     * multiple processes */
    pthread_mutexattr_init(&bufptr->mutexattr);
    pthread_mutexattr_setpshared(&bufptr->mutexattr, PTHREAD_PROCESS_SHARED);
    pthread_mutex_init(&bufptr->countermutex, &bufptr->mutexattr);
...
static void closebuffer(sharedbuffer_t *bufptr) {
    if (NULL != bufptr) {
        pthread_mutexattr_destroy(&bufptr->mutexattr);
        pthread_mutex_destroy(&bufptr->countermutex);
    }
    shm_unlink(shname);        
}
...
static void producer() {
    ...
    for (i=0; i<NUM_PRODUCED; i++) {
        ...
        pthread_mutex_lock(&bufptr->countermutex);
        bufptr->counter ++; 
        pthread_mutex_unlock(&bufptr->countermutex);
