...
int main(int argc, char *argv[]) {
  ...
  if (-1 == write(pipefd[1], &msglen, sizeof(msglen)))
    errExit("p: write");
  if (-1 == write(pipefd[1], argv[1], strlen(argv[1])))
    errExit("p: write");

  if (read(pipefd[2], &msglen, sizeof(msglen)) != sizeof(msglen))
    errExit("p: read");
  ...
  if (waitpid(pid, NULL, 0) == -1)
    errExit("waitpid"); /* Wait for child */
