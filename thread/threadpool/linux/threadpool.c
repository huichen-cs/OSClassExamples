...
thread_pool_t *thread_pool_create(int num_threads, int max_task_count) {
  ...
  thread_pool->task_list =
      (thread_pool_task_t *)malloc(max_task_count * sizeof(thread_pool_task_t));
  ...
  for (i = 0; i < num_threads; i++) {
    if (pthread_create(&(thread_pool->tid[i]), NULL, thread_pool->runner,
                       thread_pool) != 0) {
...
void thread_pool_add_task(thread_pool_t *thread_pool,
  ...
  pthread_mutex_lock(&thread_pool->mutex_task_list);
  thread_pool_task_t task = {.task_routine = task_routine, .args = args};
...
void thread_pool_wait(thread_pool_t *thread_pool) {
  pthread_mutex_lock(&thread_pool->mutex_task_list);
  while (1) {
    // printf("entering thread_pool_wait loop can_exit = %d, task_count = %d,
    // thread_count = %d\n",
    //         thread_pool->can_exit, thread_pool->task_count,
    //         thread_pool->thread_count);
    if ((!thread_pool->can_exit && thread_pool->task_count != 0) ||
        (thread_pool->can_exit && thread_pool->thread_count != 0)) {
      pthread_cond_wait(&thread_pool->cond_can_exit,
                        &thread_pool->mutex_task_list);
    } else {
      // printf("exiting thread_pool_wait loop can_exit = %d, task_count = %d,
      // thread_count = %d\n",
      //     thread_pool->can_exit, thread_pool->task_count,
      //     thread_pool->thread_count);
      break;
