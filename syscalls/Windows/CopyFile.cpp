...
static void ExitIfFailed(char *pathname, HANDLE hFile) {
  if (hFile == INVALID_HANDLE_VALUE) {
    ...
    std::cerr << "When " << pathname
              << " being opened error occured with error no: " << errorCode
              << " and error message: " << (LPCTSTR)lpMsgBuf << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
static void ExitIfFailed(char *srcpathname, char *dstpathname, int status) {
  if (status == -1) {
    ...
    std::cerr << "When " << srcpathname << " was being copied to "
              << dstpathname << " error occured with error no: " << errorCode
              << " and error message: " << (LPCTSTR)lpMsgBuf << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
static int CopyFile(HANDLE hSrcFile, HANDLE hDstFile) {
  ...
  while ((readStatus =
              ReadFile(hSrcFile, buffer, BUFFER_SIZE - 1, &bytesRead, NULL))) {
    if (bytesRead == 0)
      break;
    // See
