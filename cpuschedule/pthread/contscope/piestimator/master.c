...
int main(int argc, char *argv[]) {
  ...
  for (tnum = 0; tnum < num_threads; tnum++) {
    sscanf(argv[2 + 3 * tnum], "%" SCNd64, &(tinfo[tnum].maxiter));
  ...
  for (tnum = 0; tnum < num_threads; tnum++) {
    printf("Read from the command line for thread(%d): %" SCNd64 " %d %d\n",
  ...
  if (pthread_attr_setscope(&attr, PTHREAD_SCOPE_PROCESS) != 0) {
    fprintf(stderr,
            "Unable to set scheduling scope as PTHREAD_SCOPE_PROCESS\n");
  } else {
    printf("Successfully set scheduling scope as PTHREAD_SCOPE_PROCESS\n");
  }
  ...
  for (tnum = 0; tnum < num_threads; tnum++) {
     ...
     * element of tinfo[] */
    status = pthread_create(&tinfo[tnum].thread_id, &attr, &piworker,
