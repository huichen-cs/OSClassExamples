...
public final class MainViewController {
  ...
  private static final int BUF_SIZE = 16; // TO DO: experiment with buffer size
  ...
  private void copyFile() {
    ...
    try (FileChannel inChannel = FileChannel.open(srcFilePath, StandardOpenOption.READ);
      ...
      // TO DO: add notification: copy failed
    }
  }

  private void cancelCopyFile(final FileChannel channel) {
    try {
      if (channel.isOpen()) {
        channel.close();
      }
      ...
      LOGGER.error(e.getMessage(), e);
      // TO DO: add notification: cancel may not be successful
    }
  }
  ...
  private void setUIForCopyInProgress(final FileChannel channel) {
    copyButton.setDisable(true);
