...
public final class MainViewController {
  ...
  @FXML private Button copyFromButton;

  @FXML private Button copyToButton;

  @FXML private TextField copyFromField;

  @FXML private TextField copyToField;

  @FXML private ProgressBar copyingProgress;

  @FXML private Button cancelButton;

  @FXML private Button copyButton;

  @FXML private Label copyingInfoLabel;
  ...
  private void setUIForNextCopy(final String msg) {
  ...
  private void startAioCopyFile() throws IOException {
    ...
    outChannel =
        AsynchronousFileChannel.open(
            dstFilePath, StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE);
    ...
    synchronized (this) {
      if (inChannel != null && inChannel.isOpen()) {
        inChannel.read(byteBuffer, filePosition, byteBuffer, inHandler);
      }
    }
  }
  ...
  private final class ReadCompletionHandler implements CompletionHandler<Integer, ByteBuffer> {
    ...
    @Override
    public void completed(final Integer result, final ByteBuffer byteBufferRead) {
      ...
      synchronized (this) {
        if (outChannel != null && outChannel.isOpen()) {
          outChannel.write(byteBufferRead, filePosition, new Long(filePosition), outHandler);
        }
      }
      ...
      if (filePosition + result.intValue() < fileSize) {
        ...
        synchronized (this) {
          if (inChannel != null && inChannel.isOpen()) {
            inChannel.read(
                byteBufferToRead, filePosition + result.intValue(), byteBufferToRead, inHandler);
          }
        }
      }
    }
    ...
    @Override
    public void failed(final Throwable e, final ByteBuffer attachment) {
      // TODO deal with the error
      LOGGER.error("Failed to read the file", e);
    }
  }

  private final class WriteCompletionHandler implements CompletionHandler<Integer, Long> {
